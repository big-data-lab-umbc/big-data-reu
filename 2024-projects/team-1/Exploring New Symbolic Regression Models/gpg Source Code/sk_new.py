...
class GPGRegressor(BaseEstimator, RegressorMixin):
    ...
    def fit(self, X, y, label_indices=None, Z=None, loss_method='regular', cluster_labels=None, weight=1,
            bounds=(1, 3.05, 9.15, 102)):
        # setup cpp interface
        ...
        if isinstance(y, pd.Series):
            y = y.values
        if isinstance(label_indices, pd.Series):
            label_indices = label_indices.values
        ...
        self.model = self._pick_best_model(X, y, models, label_indices, Z, loss_method, cluster_labels, weight, bounds)

    def _finetune_multiple_models(self, models, X, y, label_indices, Z, loss_method, cluster_labels, weight, bounds):
        ...
        if len(X) > 10000:
            ...
            for i, m in enumerate(models):
                models[i], steps_done = ft.finetune(m, X, y, n_steps=finetune_num_steps[i], label_indices=label_indices,
                                                    Z_vals=Z, loss_method=loss_method, cluster_labels=cluster_labels,
    ...
    def _pick_best_model(self, X, y, models, label_indices, Z, loss_method, cluster_labels, weight, bounds):

        # simplify (with stopping), simplify the equations
        if hasattr(self, "verbose") and self.verbose:
        ...
        if hasattr(self, "finetune") and self.finetune:
            self._finetune_multiple_models(models, X, y, label_indices=label_indices, Z=Z, loss_method=loss_method,
                                           cluster_labels=cluster_labels, weight=weight, bounds=bounds)
        ...
        for i, m in enumerate(models):
            ...
            if loss_method == 'binned-rainfall':
                ...
                err = mean_squared_error(y, p) + weight * (relu0_0 + relu0_1 + relu1_0 + relu1_1 + relu2_0 + relu2_1)
            # Z-R loss
            if loss_method == 'Z-R':
                a = 134
                b = 1.6
                err = 1 * mean_squared_error(y, p) + weight * mean_squared_error(p, (Z / a) ** (1 / b))
            # Cluster-based loss
            if loss_method == 'clusters':
                # Compute silhouette score
                sil_score = silhouette_score(X=p.reshape(-1, 1), labels=cluster_labels)
                err = 1 * mean_squared_error(y, p) - weight * sil_score
            if err > max_err:
                max_err = err
            errs.append(err)
        # adjust errs
