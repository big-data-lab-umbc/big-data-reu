...
#SBATCH --constraint=rtx_8000   # for now, we are doing all prediction runs on an rtx_6000 node
#SBATCH --error=slurm_output/slurm_pred.err
#SBATCH --output=slurm_output/slurm_pred.out

# variables
run_id='mc7.17_imFCNv1'  # CHANGE THIS
...
ls
ls | grep '^version_[0-9]\+$' | cut -c2- | sort -n | tail -n1 | sed 's|^|v|'
