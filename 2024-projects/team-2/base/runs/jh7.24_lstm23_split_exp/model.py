...
class LSTM2023(pl.LightningModule):
    def __init__(self, indim, outdim, num_layers, neurons, lr, lr_step, lr_gam, dropout, activation, optimizer, l2=0):
        super().__init__()
        self.save_hyperparameters()
...
class ImprovedFCN(pl.LightningModule):    
    ...
    def loss_fn(self, x, y):
        ...
        pair_wise = self.loss_penalty(pair_wise_x, y) ** self.penalty  # put in actual predictions for custom loss
        # def figure out above softmax mode, not argmax but hard mode
        return cross_entropy * (1 + pair_wise)
...
# FCN + CNN + LSTM experimentation, implement loss function regularization and early stopping
class MultiType(pl.LightningModule):    
    def __init__(self, input_size, num_classes, fcn_layers, activation, optimizer, l2, lr, lr_step, lr_gam, loss_penalty, dropout=None):
        super().__init__()
        self.save_hyperparameters()
