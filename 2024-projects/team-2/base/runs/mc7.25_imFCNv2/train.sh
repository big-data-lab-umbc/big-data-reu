#!/bin/bash
#SBATCH --job-name=FCNv2                                
#SBATCH --mem=48G                                   
#SBATCH --nodes=2                # num nodes: MUST match .yaml file
#SBATCH --gres=gpu:3             # num gpus per node: MUST match .yaml file AND ntasks-per-node=
#SBATCH --ntasks-per-node=3      # num gpus per node: MUST match .yaml file AND gres=gpu:
...
# variables
run_id='mc7.25_imFCNv2'  # CHANGE THIS
