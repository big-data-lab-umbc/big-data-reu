...
'''
# Determine appropriate values for variance and length scale for each variable in the dataset
variances = []
len_scales = [25_000, 25_000, 50_000, 75_000, 175_000] # Estimated based on visualization

for ind, var_vals in enumerate(vars):
  # Variance for current var
  variances.append(np.var(var_vals))

  # Plot heatmap of variable over x,y
  print(var_vals.shape)
  plotHeatMap(var_vals, x, y, varNames[ind])

  # Length scale...CHANGE THIS
  #len_scales.append(1.0)
'''
...
## Todo: split into axn batches of prediction grids and training data based on spatial information
...
# x_fix, y_fix = surf_x and surf_y (as pts)
# p_x, p_y = track_x, track_y (as pts.) I.e. pts where we want to make prediction
...
# kriging_out_M[i_yBatch,i_xBatch] = (mean_field, var_field)

# Visualize mean fields from kriging_out_M